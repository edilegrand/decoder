...
import { Search, Bell, User, Hexagon, Menu, X, LogOut } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const { user, isAdmin, nickname, logout } = useAuth();
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-white/10 bg-slate-950/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        {user ? (
          ...
          <button onClick={() => setIsLoginModalOpen(true)} className="flex items-center gap-2 group">
            <motion.div
              whileHover={{ rotate: 180 }}
              transition={{ duration: 0.5 }}
            >
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-slate-400 hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/30 transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-1.5 text-sm text-cyan-400 hover:bg-cyan-500/20 transition-colors"
              >
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-4 py-4 space-y-4">
              ...
              {user ? (
                <>
                  <div className="flex items-center gap-3 text-white p-2 rounded">
                    <User className="h-5 w-5" />
                    <div className="flex flex-col">
                      <span className="text-sm font-medium">{nickname || user.email?.split('@')[0] || 'User'}</span>
                      <span className="text-xs text-slate-400 truncate">{user.email}</span>
                    </div>
                  </div>
                  <button
                    onClick={handleLogout}
                    className="flex items-center gap-3 w-full text-red-400 p-2 hover:bg-red-500/10 rounded"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-3 w-full text-cyan-400 p-2 hover:bg-cyan-500/10 rounded"
                >
                  <User className="h-5 w-5" />
                  <span>Login</span>
                </button>
